...
async def logout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "👋 Halo! Kirim token Grab-mu ke sini, dan akan di bantu logout akun kamu."
    )
...
async def handle_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if response.status_code == 200:
            await update.message.reply_text("✅ Logout berhasil! Token Blank 😜")
...
if __name__ == "__main__":
    TELEGRAM_TOKEN = "8871195550:AAGJZ_a2KJ4vlnRmQ1hi2uVQGw5BlwprXt8"  # Ganti dengan token bot Telegrammu
